...
export class ThreadSource {
    static init(messagesService: MessagesService, threadService: ThreadService, usersService: UsersService): void {
    ...
    static setupBots(messagesService: MessagesService): void {
        messagesService.messagesForThreadUser(tEcho, echo)
            .forEach((message: Message): void => {
                messagesService.addMessage(new Message({ author: echo, text: message.text, thread: tEcho }));
            }, null);

        messagesService.messagesForThreadUser(tRev, rev).forEach((message: Message): void => {
            messagesService.addMessage(
                new Message({ author: rev, text: message.text.split('').reverse().join(''), thread: tRev })
            );
        }, null);
